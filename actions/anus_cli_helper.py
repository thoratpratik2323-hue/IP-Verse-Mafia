...
def anus_cli_helper(parameters: dict, player=None) -> str:
    """
    Main dispatcher called from main.py.
    
    parameters:
        action      : autonomous_run | solana_balance | solana_transfer | solana_history
        goal        : Target objective for autonomous terminal run
        max_steps   : Maximum command iterations for autonomous run (default: 5)
        target      : Target recipient address for SOL transfers
        amount      : Solana amount to transfer
    """
    p = parameters or {}
    ...
    if action == "autonomous_run":
        ...
        return f"Invalid ANUS CLI action: '{action}', sir."
